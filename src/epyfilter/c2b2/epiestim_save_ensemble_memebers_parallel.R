...
#' @Description: Run EpiEstim to save 
################################################################################
rm(list=ls())
...
parser$add_argument("--param-list", nargs='+', default = c(6981), help = "rerun for specific params", type = "integer")
...
for (file in files) {
  ...
  # save epiestim posterior samples
  fwrite(R_posterior_all_dt, file=paste0(out_dir,"/",param_num, "_epiEstim_post.csv"))
}
print("DONE!")